...
Usage:
    vulnerability-scan [path] [--api-key gsk_...] [--report]
    python -m vul_scan [path]
"""
import os
import sys

# ── Pre-parse --api-key BEFORE package imports ────────────────────────────────
# config.py validates GROQ_API_KEY at import time, so we must inject it early.
def _inject_api_key_from_argv() -> None:
    args = sys.argv[1:]
    for i, arg in enumerate(args):
        if arg == "--api-key" and i + 1 < len(args):
            os.environ["GROQ_API_KEY"] = args[i + 1]
            return
        if arg.startswith("--api-key="):
            os.environ["GROQ_API_KEY"] = arg.split("=", 1)[1]
            return
...
def main():
    ...
    parser.add_argument(
        ...
        nargs="?",
        default="./project",
        help="File or directory to scan (default: ./project)",
    )
    parser.add_argument(
        "--api-key",
        metavar="KEY",
        help="Groq API key (overrides GROQ_API_KEY env var / .env file)",
    )
    parser.add_argument(
        "--report",
        action="store_true",
        help="Also generate a Professional_Vulnerability_Report.docx",
    )
